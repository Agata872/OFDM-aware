...
model_path = os.path.join(script_dir, 'saved_model', 'UserCentricDNN_cell{}_Nc{}_M{}_B{}_K{}'.format(Ball, Nc, M, B, K))
pathlib.Path(model_path).mkdir(parents=True, exist_ok=True)
checkpoint_path = os.path.join(model_path, 'DNN_local.zip')

DNN_local = FCNN().to(device)
print("DNN Local CSI: ", DNN_local)

if initial_run == 0 and os.path.exists(checkpoint_path):  # continue to train
    DNN_local.load_state_dict(torch.load(checkpoint_path, map_location=torch.device(device)))
elif initial_run == 0:
    print(f'Checkpoint not found at {checkpoint_path}; continuing with randomly initialized weights.')
