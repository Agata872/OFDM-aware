...
batch_per_epoch = 50
num_epochs = 5
initial_run = 1  # 0: resume GRU training from checkpoint, 1: train GRU from scratch
...
dnn_model_path = os.path.join(script_dir, 'saved_model', 'UserCentricDNN_cell{}_Nc{}_M{}_B{}_K{}'.format(Ball, Nc, M, B, K))
gru_model_path = os.path.join(script_dir, 'saved_model', 'UserCentricDNN_GRU_cell{}_Nc{}_M{}_B{}_K{}_step{}'.format(Ball, Nc, M, B, K, time_step_load))
pathlib.Path(gru_model_path).mkdir(parents=True, exist_ok=True)

dnn_checkpoint_path = os.path.join(dnn_model_path, 'DNN_local.zip')
gru_checkpoint_path = os.path.join(gru_model_path, 'GRU.zip')

DNN_local = FCNN().to(device)
print("DNN Local CSI: ", DNN_local)
if not os.path.exists(dnn_checkpoint_path):
    raise FileNotFoundError(
        f'Stage-1 checkpoint not found: {dnn_checkpoint_path}. '
        f'Please run LocalCSI_DNN.py first to generate DNN_local.zip.'
    )
DNN_local.load_state_dict(torch.load(dnn_checkpoint_path, map_location=torch.device(device)))
...
if initial_run == 0:  # continue to train
    if os.path.exists(gru_checkpoint_path):
        gru_meta.load_state_dict(torch.load(gru_checkpoint_path, map_location=torch.device(device)))
    else:
        print(f'GRU checkpoint not found at {gru_checkpoint_path}; start training GRU from scratch.')
...
for ee in range(num_epochs):
    ...
    if (ee+1) % 5 == 0 and rate_val_list[-1] > best_rate_val:
        ...
        ### save the best model
        # torch.save(DNN_local.state_dict(), dnn_checkpoint_path)
        torch.save(gru_meta.state_dict(), gru_checkpoint_path)
...
with torch.no_grad():
    ...
    result_path = os.path.join(script_dir, 'plot_result', 'plot_convergence',
